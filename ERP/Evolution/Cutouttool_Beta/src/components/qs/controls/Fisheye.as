package components.qs.controls
{
	import components.qs.controls.fisheyeClasses.FisheyeBase;
	import components.qs.controls.fisheyeClasses.FisheyeItem;
	import components.qs.controls.fisheyeClasses.FisheyeAxis;
		...
		*  we go through these properties. Switch the properties, and our layout switches axis. Presto! */
		...
		override protected function generateLayout():void
		{
			...
			else
			{
				...
				for (var j:int = targetIndex;j<itemCount;j++)
					_pdata[j][major.pos] += targetPosition;
				if(targetIndex > 0)
				{
					...
					for(var k:int = 0;k<targetIndex;k++)
						_pdata[k][major.pos] += offset;
				}				
			}
