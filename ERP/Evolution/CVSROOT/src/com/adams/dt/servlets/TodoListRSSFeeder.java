...
public class TodoListRSSFeeder extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException { 
		...
		String enNavUrl = request.getParameter("eurl")!=null?request.getParameter("eurl"):"";
		
		System.out.println("enUsername:"+enUsername);
		System.out.println("enUsername:"+enPassword);
		System.out.println("enNavUrl:"+enNavUrl);
		
		try {		

			String tempUsername = encryptUtil.getDecryptedString(enUsername);
			String tempPassword = encryptUtil.getDecryptedString(enPassword);
			System.out.println("tempUsername:"+tempUsername);
			System.out.println("tempPassword:"+tempPassword);
			
			String encryptedUserString = encryptUtil.getEncryptedString(tempUsername);
			String urlEncodedUserString = URLEncoder.encode(encryptedUserString, "UTF8");
			System.out.println("encryptedUserString:"+encryptedUserString);
			System.out.println("urlEncodedUserString:"+urlEncodedUserString);
			
			String encryptedPassString = encryptUtil.getEncryptedString(tempPassword);
			String urlEncodedPassString = URLEncoder.encode(encryptedPassString, "UTF8");
			System.out.println("encryptedPassString:"+encryptedPassString);
			System.out.println("urlEncodedPassString:"+urlEncodedPassString);
			
			BuildConfig.flexUsername = urlEncodedUserString;
			BuildConfig.flexPassword = urlEncodedPassString;
			BuildConfig.navigationHtmlUrl = enNavUrl;
	...
	private void assignTaskEntry(Object[] objs) {
		...
		System.out.println("BuildConfig.flexUsername :"+BuildConfig.flexUsername);
		System.out.println("BuildConfig.flexPassword :"+BuildConfig.flexPassword);
		System.out.println("BuildConfig.navigationHtmlUrl :"+BuildConfig.navigationHtmlUrl);
		//%23ampuser="+userId+"%23amppass="+password+"
		try {
			entry = new SyndEntryImpl();
			description = new SyndContentImpl();
			descriptionStr = "";
			...
			entry.setLink("http://"+BuildConfig.serverAddress+":"+BuildConfig.serverPort+"/"+BuildConfig.rssRootContext+"/"+BuildConfig.navigationHtmlUrl+"?type=All%23ampuser="+BuildConfig.flexUsername+"%23amppass="+BuildConfig.flexPassword+"%23amptaskId="+objs[3]);
