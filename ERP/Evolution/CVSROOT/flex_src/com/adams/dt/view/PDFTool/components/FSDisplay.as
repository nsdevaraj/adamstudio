package com.adams.dt.view.PDFTool.components
{
	...
	public final class FSDisplay extends Canvas
	{
		...
		{
			super();
			addEventListener(FlexEvent.CREATION_COMPLETE , initComponent,false,0,true);
		}
		...
		private function initComponent(e : FlexEvent) : void
		{
			...
			systemManager.stage.addEventListener(FullScreenEvent.FULL_SCREEN , fullScreenHandler,false,0,true);
			...
			_componentInst.styleName ='fullscreenBgColor'; 
		...
		*/		
		...
		private function fullScreenHandler(e : FullScreenEvent) : void
		{
			if(ScreenStatus){
				if (e.fullScreen)
				{
					_componentInst.setStyle('backgroundAlpha',1)
				...
				{
					if(model.CF!=1){
						_componentInst.setStyle('backgroundAlpha',0)
					}
