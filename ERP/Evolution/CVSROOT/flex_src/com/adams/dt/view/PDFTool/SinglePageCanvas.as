package com.adams.dt.view.PDFTool
{
	...
	public class SinglePageCanvas extends Canvas
	{
		...
		public function onZoom(zoomIn:Boolean = true):void{
			pagePositionToCenter();
			var transMatrix:Matrix =transform.matrix;
			if(zoomIn)
			{
				if((imgWidth * rotateScaleValue())<imgWidth*15 && (imgHeight * rotateScaleValue())<imgHeight*15)
				{
			...
			{ 
				if((imgWidth * rotateScaleValue())>50 && (imgHeight * rotateScaleValue())>50)
				{
			...
			if(panSelect)
			{
				...
				if(this.focusAction==NOTE && !this.dragStart && drawingStart)
				{
					if(Math.abs(startX-stopX) > minimumNoteWidth || Math.abs(startY-stopY) > minimumNoteHeight)
					{
						...
						model.pdfDetailVO.notesModificationHistoryAC.addItem({name:model.person.personFirstname +"( "+ (GetVOUtil.getProfileObject(model.person.defaultProfile).profileLabel.substr(0,3)) +" )",title:"",desc:"", status: "New Comment"});
        ...
        public function rotateZero():void
        {
        	...
        	var tempTransform:Matrix= new Matrix();
        	tempTransform = transform.matrix
        	tempTransform.a = tempTransform.d = this.parent.height/(imgHeight+100);
        	tempTransform.b = tempTransform.c = 0;  
		...
		public function drawFromString(value:String, commentObj:DrawingSprite):void{
				...
				if(shape=='ellipse'){
					...
					if(commentObj.pdf == SinglePageCanvas.PDF2)
					{
						DrawNoteCircle(cx-rx, cy-ry/2,1);
					}
				}    
				if(shape=='path'){
					var pValue:String;
