	...
	public class HtmlGenerator
	{
		...
		public function createHtml( tableColl:ArrayCollection , projectsComments:String ):void
		{
			...
			for each( var exportHtmlObj:Object in tableColl)
		  	{
		  		if(CheckBox( exportHtmlObj.checkBoxSelected ).selected == true)
				{
		  			generateGrid(exportHtmlObj.properties,exportHtmlObj.Category)
		 		}
		  	}
			var header:String = "<br/><br/>"+model.currentProjects.projectName+"<br/><br/>"+"<p>"+Utils.getDomains(model.currentProjects.categories).categoryName+"<span></span>"+( model.currentTime ).toDateString()
								+"</p><br></br>"
