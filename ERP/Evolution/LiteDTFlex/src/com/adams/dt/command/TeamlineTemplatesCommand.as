...
package com.adams.dt.command
{
	...
	{ 
		...
		override public function execute( event : CairngormEvent ) : void {	 
		    ...
		    switch(event.type){    
	            ...
	            case TeamlineTemplatesEvent.EVENT_SELECT_TEAMLINETEMPLATES:
	            	delegate.responder = new Callbacks( findByTeamTempResult, fault );
	             	delegate.findById( teamlineTemplatesEvent.teamtemplates.teamTemplateId );
	            break;  
	            default:
	            break; 
		...
		private function findByTeamTempResult( rpcEvent:Object ):void { 
		...
		private function findByIdResult( rpcEvent:Object ):void {
			...
			if( model.referenceProject ) {
				for each( var team:Teamlines in model.referenceTeamline ) {
					...
					referTeamLines.projectID = model.currentProjects.projectId; 
			...
			if( model.clientTeamlineId != 0 ) {
				...
				clientTeamline.projectID = model.currentProjects.projectId;
			...
			if( model.currentImpremiuerID != 0 ) {
				...
				teamLineSep.projectID = model.currentProjects.projectId;
			...
			if( model.currentUserProfileCode != 'TRA' ) {
				...
				cpp_Person.projectID = model.currentProjects.projectId;
