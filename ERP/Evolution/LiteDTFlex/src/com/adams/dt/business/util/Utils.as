package com.adams.dt.business.util
{
	...
	public class Utils
	{
		...
		public static const sortYearColl:ArrayCollection= new ArrayCollection([{Label:"2007"},{Label:"2008"},{Label:"2009"},{Label:"2010"},{Label:"2011"}]);
		...
		public static function updateCurrentProject( prj:Projects ):Projects {
			if( model.currentProjects.taskDateStart ) {
				prj.taskDateStart = model.currentProjects.taskDateStart;
			}	
			if( model.currentProjects.taskDateEnd ) {
				prj.taskDateEnd = model.currentProjects.taskDateEnd;
			}	
			if( model.currentProjects.finalTask ) {
				prj.finalTask = model.currentProjects.finalTask;
			}
			return prj;
		}
		...
		public static function setChartProvider( str:String, sourceAc:ArrayCollection ):ArrayCollection {
			var resultArrayCollection:ArrayCollection = new ArrayCollection();  
		 	if( sourceAc != null ){
				...
				var colStr:String
				for each (var col:Columns in model.totalColumnsColl){
					if(col.columnName == str){
						colStr = col.columnField;
						break;
					}
				}
				for ( var i:int = 0; i < sourceAc.length; i++ ) {
					...
					if( sourceAc.getItemAt( i ) != null ) {
						var storedValue:String = sourceAc.getItemAt( i )[ colStr ];
		...
		public static function createTaskMail(teamLineCollection:ArrayCollection,init:Boolean =false):void{ 
        	...
        	{     	
        		...
        		if(init){
        			var workflowTemplate:Workflowstemplates = getWorkflowTemplates(model.modelAnnulationWorkflowTemplate,model.currentProjects.workflowFK);
	        		...
	        		projectName = model.currentProjects.projectName;
	  ...
	  public static function getOPEHideTasks( value:int ):Boolean {
		   for each( var item:Tasks in model.currentProjectTasksCollection ) {
		   		trace("UTILS getOPEHideTasks :"+item.taskId);
		   		if(item.taskId == value){
		   		trace("UTILS item.workflowtemplateFK.taskCode :"+item.workflowtemplateFK.taskCode);		   			
			    	/* if( (item.workflowtemplateFK.taskCode == "P3T02A") || 
			    		(item.workflowtemplateFK.taskCode == "P3T02B") ||
			    		(item.workflowtemplateFK.taskCode == "P4T03A") ||
			    		(item.workflowtemplateFK.taskCode == "P4T03B") ) {
			     			return true;
			    	} */
			    	 if( (item.workflowtemplateFK.taskCode == "P1T02A") ||
			    	 	(item.workflowtemplateFK.taskCode == "P4T02A") ||
			    	 	(item.workflowtemplateFK.taskCode == "P5T01A") ||
			    	 	(item.workflowtemplateFK.taskCode == "P3T02A") || 
			    		(item.workflowtemplateFK.taskCode == "P3T02B") ||
			    		(item.workflowtemplateFK.taskCode == "P4T03A") ||
			    		(item.workflowtemplateFK.taskCode == "P4T03B") ) {
			     			return true;
	  ...
	  public static function checkDuplicateReportItem( item:Object, dp:ArrayCollection, sortString:String ):Boolean {
		...
		public static function updatePropertiesPj( fieldArray:Array , presetFkArray:Array , propertiesPjId:Array ):void
		{
			model.propertiespjCollection = new ArrayCollection();
    		for (var i:int =0;i<presetFkArray.length;i++)
    		{
	    			var newPropPj:Propertiespj = new Propertiespj();
	    			newPropPj.propertyPjId = propertiesPjId[i];
					newPropPj.fieldValue = (fieldArray[i]!=null)?fieldArray[i]:' ';
					newPropPj.projectFk= model.currentProjects.projectId;
					newPropPj.propertyPreset = GetVOUtil.getPropertiesPresetObject(presetFkArray[i])
					model.propertiespjCollection.addItem(newPropPj);    			
    		}
    		model.currentProjects.propertiespjSet = model.propertiespjCollection;
		}
		...
		public static function makePhasesSet( workflowFk:int, startTime:Date ):ArrayCollection {
			var phasesCollection:ArrayCollection = new ArrayCollection();
			for( var i:int = 0; i < model.allPhasestemplatesCollection.length; i++ ) {
				var item:Phasestemplates = model.allPhasestemplatesCollection.getItemAt( i ) as Phasestemplates;
		...
		public static function getIntialPhases(phasesColl:ArrayCollection,workflow_Id:int):ArrayCollection
		{
			...
			var sort:Sort = new Sort();
			sort.fields = [new SortField('phaseTemplateId')];
			model.allPhasestemplatesCollection.sort = sort;
			model.allPhasestemplatesCollection.refresh();
		...
		public static function traceLog(text:String):void
		{
			var os:String = Capabilities.os.substr(0, 3);
			var fullPath:String;
			if (os == "Win") {
				fullPath = File.userDirectory.nativePath+"/DTFlex/DTFlexLog.txt";
			} else if (os == "Mac") {
				fullPath = "file://"+File.userDirectory.nativePath+"/DTFlex/DTFlexLog.txt";
			}
			var file:File = new File(fullPath);
			var fileStr:FileStream = new FileStream();
			var fileMode:String = (!file.exists)?FileMode.WRITE:FileMode.APPEND;
			var nowDate:Date = new Date();
			var logText:String =  "===========================================" +File.lineEnding+ nowDate.date+"/"+nowDate.month +"/"+nowDate.fullYear+ " | " + nowDate.hours + ":" + nowDate.minutes + ":" + nowDate.seconds + File.lineEnding + "( Log )" +text + File.lineEnding+"-------------------------------------------"+File.lineEnding+File.lineEnding+File.lineEnding;
			fileStr.open(file,fileMode);
			fileStr.writeMultiByte( logText, File.systemCharset );
			fileStr.close();
		}
		
		public static function getDTLiteCltPersons( str:String ):Array {
			var arr:Array = [];
			for each( var team:Teamlines in model.teamlLineCollection ) {
				if( team.profileID == getProfileId( str ) ) {
					arr.push( getPersonOnFilterApply( team.personID ) );
				}
			}
			return arr;
		}
		
		public static function getMessageTemplate( pro:int ):Workflowstemplates {
			var workTemp:Workflowstemplates;
			var messageTemplateCollection:ArrayCollection = getWorkflowTemplatesCollection( model.messageTemplatesCollection, model.currentProjects.workflowFK );
