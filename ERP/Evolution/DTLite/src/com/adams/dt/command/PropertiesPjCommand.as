package com.adams.dt.command
{
	...
	{ 
		...
		override public function execute( event:CairngormEvent ):void {	 
			...
			switch( event.type ) {    
			    ...
			    case PropertiespjEvent.EVENT_UPDATE_PROPERTIESPJ:
			    	...
			    	delegate.createProjectProperties(model.currentProjects.projectId.toString(),String(pjresult.split("#&#")[1]).slice(0,-1),String(pjresult.split("#&#")[0]).slice(0,-1));
			    break; 
			    ...
			    break; 
			    case PropertiespjEvent.EVENT_GET_PROPERTIESPJ_BY_PROJECT:
			    	 delegate.responder = new Callbacks( updateDashBoardPropertiesPJ, fault )
			    	 delegate.findProjectId( propertiespjEvent.prop_ProjectID );
			    break;
			    ...
			    break;
			    case PropertiespjEvent.EVENT_ORACLE_BULKUPDATE_PROPERTIESPJ:
		       		this.delegate = DelegateLocator.getInstance().pagingDelegate;
		       		this.delegate.responder = new Callbacks(oracleBulkPropertiesPjResult,fault);	       		
		       		this.delegate.createProjectProperties(propertiespjEvent.prop_projectId,propertiespjEvent.prop_presetId,propertiespjEvent.prop_prefieldvalue);		       		
		       	break;  
			} 
		}
		private function updateDashBoardPropertiesPJ( rpcEvent:Object ):void {	
			model.propertiespjCollection = rpcEvent.result as ArrayCollection;
			model.currentProjects.propertiespjSet = model.propertiespjCollection;
			super.result( rpcEvent );
		}
		public function propertiesSPJ():ArrayCollection{
			var returnarrayCol:ArrayCollection = new ArrayCollection();
			for each(var proppj:Propertiespj in model.propertiespjCollection){
				var newPropPj:Propertiespj = new Propertiespj();
					if(proppj.fieldValue!=null)
				 		newPropPj.fieldValue = proppj.fieldValue;
				 	else 
				 		newPropPj.fieldValue = '';
				 		
				 newPropPj.projectFk= model.currentProjects.projectId;
				 newPropPj.propertyPreset = proppj.propertyPreset;
				 returnarrayCol.addItem(newPropPj);
			} 
			return returnarrayCol;
		}
		private function oracleBulkPropertiesPjResult( rpcEvent:Object ):void {	
			super.result( rpcEvent );
		...
		private function deportUpdateResult( rpcEvent:Object ):void {			
			//31 DEC 2010
			/* Utils.updatePropertiesPj(( rpcEvent.result ).getItemAt( 0 ) as Array , 
			( rpcEvent.result ).getItemAt( 1 ) as Array,( rpcEvent.result ).getItemAt( 2 ) as Array);
			 */
		...
		private function updateResult( rpcEvent:Object ):void {
			...
			/* if( model.currentProjects.propertiespjSet.length == ArrayCollection( rpcEvent.message.body ).length )
				...
				model.currentProjects.propertiespjSet = Utils.modifyItems( model.currentProjects.propertiespjSet, model.currentProjects.propertiespjSet, 'propertyPjId' ); 
			 */
				super.result( rpcEvent );
