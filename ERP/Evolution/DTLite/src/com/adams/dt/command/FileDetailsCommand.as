package com.adams.dt.command
{
	...
	{ 
		...
		override public function execute( event:CairngormEvent ):void
		{
	       ...
	       switch( event.type ) {  
	            ...
	            case FileDetailsEvent.EVENT_GET_TASKFILEDETAILS:
	            	...
	            	trace("EVENT_GET_TASKFILEDETAILS :"+model.currentProjects.projectId);
		...
		public function releaseFileDetailsResult( rpcEvent:Object ):void {
	        ...
	        trace("\n\n ********************************************* FILESDETAILSCOMMAND releaseFileDetailsResult :"+model.taskFileCollection.length+" -model.trafficOnly- "+model.trafficOnly);
            if(model.createdTask){
				if(model.createdTask.previousTask){
	            	trace(" FILESDETAILSCOMMAND createdTask previousTask :"+model.createdTask.previousTask.taskId);
	            	if(model.createdTask.previousTask.fileObj){
	            		trace(" FILESDETAILSCOMMAND createdTask previousTask fileObj :"+model.createdTask.previousTask.fileObj.fileId);
	            	}
	            }
	        }	
			...
			//if( model.currentUserProfileCode == 'ADM' )	 {
				trace("FILEDETAILSCOMMAND model.currentUserProfileCode :"+model.currentUserProfileCode);
			
			if( (model.currentUserProfileCode == 'ADM') || (model.currentUserProfileCode == 'TRA') )	 {
				trace("FILEDETAILSCOMMAND releaseFileDetailsResult calling");
		...
		private function createFileDetailsResult( rpcEvent:Object ):void {	
			...
			for each( var item:FileDetails in arrc ) {
				if( item.extension == "pdf" && item.type == "Tasks" ) {					
					...
					trace("********* FILEDETAILSCOMMANDS KUMAR:"+model.updateFileToTask);		
				} 
			}	
		...
		private function getBasicFileDetailsResult( rpcEvent:Object ):void {	
			...
			var handler:IResponder = new Callbacks( result, fault );
			trace("FILESDETAILSCOMMAND getBasicFileDetailsResult calling EVENT_GET_TASKFILEDETAILS")
		...
		/* private function getTaskFileDetailsResult( rpcEvent:Object ):void {
  		...
  		/* private function getTaskFileDetailsResult( rpcEvent:Object ):void {
            ...
            trace("FILESDETAILSCOMMAND getTaskFileDetailsResult :"+model.taskFileCollection.length+" -model.trafficOnly- "+model.trafficOnly);
            if(model.createdTask){
				if(model.createdTask.previousTask){
	            	trace(" FILESDETAILSCOMMAND createdTask previousTask :"+model.createdTask.previousTask.taskId);
	            	if(model.createdTask.previousTask.fileObj){
	            		trace("\n FILESDETAILSCOMMAND createdTask previousTask fileObj :"+model.createdTask.previousTask.fileObj.fileId);
	            	}
	            }
	        }
			...
			if( (model.currentUserProfileCode == "TRA") && (model.trafficOnly) ){
				trace("FILESDETAILSCOMMAND getTaskResult currentUserProfileCode:"+model.currentUserProfileCode);				
				releaseStatus();
				eventsArr.push( updateFileDetailsEvent );
				model.trafficOnly = false;
  			}	
            var filedetailsMessageEvent:FileDetailsEvent = new FileDetailsEvent( FileDetailsEvent.EVENT_GET_MESSAGEFILEDETAILS, handler );
			eventsArr.push( filedetailsMessageEvent );				  
 			var handler:IResponder = new Callbacks( result, fault )
 			var uploadSeq:SequenceGenerator = new SequenceGenerator( eventsArr, handler );
  			uploadSeq.dispatch();  
  			//******************************************************************************
		...
		private function getTaskFileDetailsResult( rpcEvent:Object ):void {
	        ...
	        //******************************************************************************
	        if( (model.currentUserProfileCode == "TRA") && (model.trafficOnly) ){
	        	if(model.releaseTasksId!=0) model.releaseTasksId = 0;

	        	trace("\n\n EVENT_GET_FILERELEASE_TASKS :"+model.createdTask.taskId+" --- "+model.createdTask.previousTask.taskId);
 	
	            var handler:IResponder = new Callbacks( result, fault );
	            var taskrelesaeEvent:TasksEvent = new TasksEvent( TasksEvent.EVENT_GET_FILERELEASE_TASKS, handler );
	            model.releaseTasksId = model.createdTask.previousTask.taskId;
				taskrelesaeEvent.dispatch(); 				
				model.trafficOnly = false;
		...
		//******************************************************************************
		private function releaseStatus():void {
			...
			trace("FILESDETAILSCOMMAND firstRelease :"+firstRelease+" , otherRelease :"+otherRelease);
			trace("FILESDETAILSCOMMAND workflowTemplateId :"+model.currentTasks.workflowtemplateFK.workflowTemplateId);
			trace("FILESDETAILSCOMMAND taskFileCollection:"+model.taskFileCollection.length);
		...
		private function updateTaskFileCollection():void{	
			...
			trace("FILESDETAILSCOMMAND updateTaskFileCollection taskFileCollection:"+model.taskFileCollection.length);
			trace("FILESDETAILSCOMMAND updateTaskFileCollection createdTask taskId:"+model.createdTask.taskId);
			for each( var item:FileDetails in model.taskFileCollection ) {
				if(model.currentTasks){
	            	trace("\n\n FILESDETAILSCOMMAND updateTaskFileCollection currentTasks :"+model.currentTasks.taskId);
	            	if(model.currentTasks.fileObj){
	            		trace(" FILESDETAILSCOMMAND updateTaskFileCollection currentTasks fileObj :"+model.currentTasks.fileObj.fileId);
	            	}
	            } 
				if(model.currentTasks.previousTask){
	            	trace(" \n\n FILESDETAILSCOMMAND updateTaskFileCollection currentTasks previousTask :"+model.currentTasks.previousTask.taskId);
	            	if(model.currentTasks.previousTask.fileObj){
	            		trace(" FILESDETAILSCOMMAND updateTaskFileCollection currentTasks previousTask fileObj :"+model.currentTasks.previousTask.fileObj.fileId);
	            	}
	            }
				trace("FILESDETAILSCOMMAND updateTaskFileCollection taskId:"+item.taskId+" , model.createdTask.previousTask.taskId :"+model.createdTask.previousTask.taskId);
				if( item.taskId == model.createdTask.previousTask.taskId ) {
					trace("FILESDETAILSCOMMAND inner 1updateTaskFileCollection item.fileId:"+item.fileId+", fileName :"+item.fileName+" , filePath :"+model.createdTask.previousTask.taskFilesPath);
					if( model.createdTask.previousTask.taskFilesPath!=null ){
						trace("FILESDETAILSCOMMAND inner fileObj item.fileId:"+item.fileId+", fileName :"+item.fileName);
						if( item.fileId == int(model.createdTask.previousTask.taskFilesPath) ){
							//trace("FILESDETAILSCOMMAND EQUAL inner 2updateTaskFileCollection item.fileId:"+item.fileId+" , model.createdTask.previousTask.fileObj.fileId :"+model.createdTask.previousTask.fileObj.fileId);
							trace("FILESDETAILSCOMMAND EQUAL inner 2updateTaskFileCollection item.fileId:"+item.fileId+" , model.createdTask.previousTask.fileObj.fileId :"+model.createdTask.previousTask.taskFilesPath);
		...
		private function miscellenousAssign(tempMiscellenous:String):void{
			if(tempMiscellenous){
				for each( var item:FileDetails in model.taskFileCollection ) {
								trace("FILESDETAILSCOMMAND miscellenousAssign item.taskId:"+item.taskId+" , model.currentTasks.previousTask.taskId :"+model.createdTask.previousTask.taskId);

					if( item.taskId == model.createdTask.previousTask.taskId ) {
									trace("FILESDETAILSCOMMAND miscellenousAssign tempMiscellenous:"+tempMiscellenous+" , item.miscelleneous :"+item.miscelleneous+" , item.visible :"+item.visible);

						if((tempMiscellenous == item.miscelleneous) && (item.visible == 0)){
									trace("FILESDETAILSCOMMAND EQUAL miscellenousAssign tempMiscellenous:"+tempMiscellenous+" , item.miscelleneous :"+item.miscelleneous+" , item.visible :"+item.visible);
		...
		/* private function {
			model.currentProjectTasksCollection
		} */	
		//******************************************************************************
