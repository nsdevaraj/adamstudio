package com.adams.dt.view.components.todolistscreens
{
	...
	public class TodoList extends Canvas implements ITodoList
	{
		...
		public function gotoNextTask():void {			
			...
			trace("TODOLIST firstRelease :"+firstRelease+" , otherRelease :"+otherRelease);
			trace("TODOLIST workflowTemplateId :"+model.currentTasks.workflowtemplateFK.workflowTemplateId);
			trace("TODOLIST taskFileCollection:"+model.taskFileCollection.length);
		...
		private function updateTaskFileCollection():void{	
			...
			trace("TODOLIST updateTaskFileCollection taskFileCollection:"+model.taskFileCollection.length);
	
			for each( var item:FileDetails in model.taskFileCollection ) {
							trace("TODOLIST updateTaskFileCollection taskId:"+item.taskId+" , model.currentTasks.previousTask.taskId :"+model.currentTasks.previousTask.taskId);

				if( item.taskId == model.currentTasks.previousTask.taskId ) {
								trace("TODOLIST inner 1updateTaskFileCollection item.fileId:"+item.fileId+" , otherRelease :"+model.currentTasks.previousTask.fileObj.fileId);

					if( item.fileId == model.currentTasks.previousTask.fileObj.fileId ){
									trace("TODOLIST EQUAL inner 2updateTaskFileCollection item.fileId:"+item.fileId+" , model.currentTasks.previousTask.fileObj.fileId :"+model.currentTasks.previousTask.fileObj.fileId);
		...
		private function miscellenousAssign(tempMiscellenous:String):void{
			if(tempMiscellenous){
				for each( var item:FileDetails in model.taskFileCollection ) {
								trace("TODOLIST miscellenousAssign item.taskId:"+item.taskId+" , model.currentTasks.previousTask.taskId :"+model.currentTasks.previousTask.taskId);

					if( item.taskId == model.currentTasks.previousTask.taskId ) {
									trace("TODOLIST miscellenousAssign tempMiscellenous:"+tempMiscellenous+" , item.miscelleneous :"+item.miscelleneous+" , item.visible :"+item.visible);

						if((tempMiscellenous == item.miscelleneous) && (item.visible == 0)){
									trace("TODOLIST EQUAL miscellenousAssign tempMiscellenous:"+tempMiscellenous+" , item.miscelleneous :"+item.miscelleneous+" , item.visible :"+item.visible);
