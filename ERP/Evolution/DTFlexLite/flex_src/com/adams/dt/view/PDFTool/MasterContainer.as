package com.adams.dt.view.PDFTool
{
	...
	public class MasterContainer extends Canvas
	{
		...
		{
			// Shared Object Work 
			sharedObjectForNotes = SharedObject.getLocal(PDFTool.pdfToolNotesSOName);
			var isNew:Boolean = (sharedObjectForNotes.data.iN)?true:false;
			// Shared Object Work
				...
				if(commentDetail.history == 0){
					...
					if(cType != SinglePageCanvas.SHAPE_NOTE){
						...
						if(isNew){
							...
							commentBox.resizeStatus = false;
							sharedObjectForNotes.data["P"+cPDF+"_N"+commentDetail.commentID] = {x:commentDetail.commentBoxX,y:commentDetail.commentBoxY,wS:0};
						}
						else
						{
							if(sharedObjectForNotes.data["P"+cPDF+"_N"+commentDetail.commentID]!=undefined){
								commentBox.x = Number(sharedObjectForNotes.data["P"+cPDF+"_N"+commentDetail.commentID].x);
								commentBox.y = Number(sharedObjectForNotes.data["P"+cPDF+"_N"+commentDetail.commentID].y);
								commentBox.resizeStatus = (sharedObjectForNotes.data["P"+cPDF+"_N"+commentDetail.commentID].wS == 1)?true:false;
								commentBox.changeSize();
							}
							else
							{
								sharedObjectForNotes.data["P"+cPDF+"_N"+commentDetail.commentID] = {x:commentDetail.commentBoxX,y:commentDetail.commentBoxY,wS:0};
						...
						// wS stands for windows Status
						//commentBox.addEventListener(CommentItem.COMMENT_MOVE, function (event:Event):void{ trace('Move');linkFunc(); },false,0,true);
						commentBox.addEventListener(CommentItem.COMMENT_MOVE, function (event:Event):void{ 
																						linkFunc();
                                                                              			var cPDF:Number = CommentBox(event.currentTarget).pdf ;
																						var id:int = CommentBox(event.currentTarget).commentVO.commentID;
																						trace('My Move'+"P"+cPDF+"_N"+id);
																						var sharedObjectForNotes:SharedObject = SharedObject.getLocal(PDFTool.pdfToolNotesSOName);
																						sharedObjectForNotes.data["P"+cPDF+"_N"+id] = {x:CommentBox(event.currentTarget).x,y:CommentBox(event.currentTarget).y,wS:((CommentBox(event.currentTarget).resizeStatus)?1:0)};
					...
					if(cPDF == SinglePageCanvas.PDF2){
						...
						commentobj.posX =  commentDetail.commentX;
						commentobj.posY =  commentDetail.commentY;
			...
			sharedObjectForNotes.data.iN = 0;
			sharedObjectForNotes.flush();
		}
