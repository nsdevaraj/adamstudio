package com.adams.dt.command
{
	...
	{ 
		    ...
		    switch(event.type){    
			   ...
			   case TasksEvent.EVENT_GET_TASK:
		       	    if( model.updateFileToTask ) {
		       	    	...
		       	    	trace("TasksEvent.EVENT_GET_TASK updateFileToTask taskId:"+model.updateFileToTask.taskId);
		       ...
		       case TasksEvent.CREATE_AUTO_INITIAL_TASKS:
		        ...
		        case TasksEvent.CREATE_COMMON_PROFILE_MSGTASKS:
		        	...
		        	if( tasksEvent.bulkMsgTaskPersonCollection.length > 0 ) delegate.create(Tasks(tasksEvent.bulkMsgTaskPersonCollection.getItemAt(0)));
		        break;
				...
				case TasksEvent.EVENT_ORACLE_NAV_CREATETASKS:
					...
					//this.delegate.createNavigationTasks(createdTask.previousTask.taskId,model.currentTasks.projectObject.workflowFK,createdTask.workflowtemplateFK.workflowTemplateId,createdTask.projectFk,model.person.personId,createdTask.workflowtemplateFK.taskCode,model.currentTaskComment.toString());
						trace("EVENT_ORACLE_NAV_CREATETASKS :"+createdTask.previousTask.taskId+" , "+model.currentTasks.projectObject.workflowFK+" , "+createdTask.workflowtemplateFK.workflowTemplateId+" , "+createdTask.projectFk+" , "+model.person.personId+" , "+createdTask.workflowtemplateFK.taskCode+" , "+model.currentTaskComment.toString());					
				...
				case TasksEvent.EVENT_STATUSCHANGE_TASK:
					...
					this.delegate.projectStatusChangeTask( tasksEvent.projectId,tasksEvent.workflowFk,tasksEvent.projectStatus,
														  tasksEvent.taskMessage,tasksEvent.personFk );
						   
					break;	
				case TasksEvent.EVENT_GET_FILERELEASE_TASKS:
					delegate.responder = new Callbacks( getTaskFileReleaseResult, fault );
					trace("\n\n EVENT_GET_FILERELEASE_TASKS :"+model.releaseTasksId);
					delegate.findByTaskId(model.releaseTasksId);
		        break;	
		...
		private function releaseStatus( tasksObj:Tasks ):void {
			...
			trace("TASKSCOMMAND firstRelease :"+firstRelease+" , otherRelease :"+otherRelease);
			trace("TASKSCOMMAND workflowTemplateId :"+model.currentTasks.workflowtemplateFK.workflowTemplateId);
			trace("TASKSCOMMAND taskFileCollection:"+model.taskFileCollection.length);

			model.release = 0;
			if( model.currentTasks.workflowtemplateFK.workflowTemplateId == firstRelease ) {
				model.release = 1; 
				updateTaskFileCollection(tasksObj);
			}
		...
		private function updateTaskFileCollection( tasksObj:Tasks ):void{	
			var tempMiscellenous:String = null;	
			trace("TASKSCOMMAND updateTaskFileCollection taskFileCollection:"+model.taskFileCollection.length);
			trace("TASKSCOMMAND updateTaskFileCollection tasksObj taskId:"+tasksObj.taskId);
			for each( var item:FileDetails in model.taskFileCollection ) {
				
				trace("TASKSCOMMAND updateTaskFileCollection taskId:"+item.taskId);
				if( item.taskId == tasksObj.taskId ) {
					trace("TASKSCOMMAND inner 1updateTaskFileCollection item.fileId:"+item.fileId+", fileName :"+item.fileName+" , fileId :"+tasksObj.fileObj.fileId+" , fileName :"+tasksObj.fileObj.fileName);
					if( tasksObj.fileObj){
						trace("TASKSCOMMAND inner fileObj item.fileId:"+item.fileId+", fileName :"+item.fileName);
						if( item.fileId == int(tasksObj.fileObj.fileId) ){
							trace("FILESDETAILSCOMMAND EQUAL inner 2updateTaskFileCollection item.fileId:"+item.fileId+" , tasksObj.fileObj.fileId :"+tasksObj.fileObj.fileId);
			...
			miscellenousAssign(tempMiscellenous,tasksObj);
		}
		private function miscellenousAssign(tempMiscellenous:String,tasksObj:Tasks):void{
			if(tempMiscellenous){
				for each( var item:FileDetails in model.taskFileCollection ) {
								trace("TASKSCOMMAND miscellenousAssign item.taskId:"+item.taskId+" , tasksObj.previousTask.taskId :"+tasksObj.taskId);

					if( item.taskId == tasksObj.taskId ) {
									trace("TASKSCOMMAND miscellenousAssign tempMiscellenous:"+tempMiscellenous+" , item.miscelleneous :"+item.miscelleneous+" , item.visible :"+item.visible);

						if((tempMiscellenous == item.miscelleneous) && (item.visible == 0)){
									trace("TASKSCOMMAND EQUAL miscellenousAssign tempMiscellenous:"+tempMiscellenous+" , item.miscelleneous :"+item.miscelleneous+" , item.visible :"+item.visible);
		...
		private function onUpload( fileObj:Object ):String {
        	...
        	trace("TASKSCOMMAND onUpload fullPath :"+fullPath);
		...
		private function getTaskResult( rpcEvent:Object ):void {	 
			...
			if( arrc ) {
				if( arrc.length > 0 ) {
					...
					/* if( (todoTskcode == "P3T02A" && tsklbl == "REALISATION V1") ||  
						(todoTskcode == "P3T02B" && tsklbl == "CORRECTIONS REALISATION V1") ||
						(todoTskcode == "P4T03A" && tsklbl == "REALISATION CORRECTIONS") || 
						(todoTskcode == "P4T03B" && tsklbl == "CORRECTIONS SUPPLEMENTAIRES")) { */
					
					if( (todoTskcode == "P1T02A" && tsklbl == "CREATION DE DOSSIER") ||
						(todoTskcode == "P1T02B" && tsklbl == "COMPLEMENTS INFOS DOSSIER") ||
						(todoTskcode == "P4T01A" && tsklbl == "RELECTURE V1") ||
						(todoTskcode == "P4T01B" && tsklbl == "COMPLEMENTS DE RELECTURE") ||
						(todoTskcode == "P4T02A" && tsklbl == "LANCEMENT CORRECTIONS") ||
						(todoTskcode == "P3T02A" && tsklbl == "REALISATION V1") ||  
						(todoTskcode == "P3T02B" && tsklbl == "CORRECTIONS REALISATION V1") ||
						(todoTskcode == "P4T03A" && tsklbl == "REALISATION CORRECTIONS") || 
						(todoTskcode == "P4T03B" && tsklbl == "CORRECTIONS SUPPLEMENTAIRES")) {
							...
							/* trace("TASKSCOMMAND getTaskFileDetailsResult :"+model.taskFileCollection.length+" -model.trafficOnly- "+model.trafficOnly);
				            if(model.updateFileToTask){
				            	trace("TASKSCOMMAND getTaskFileDetailsResult inner");
								if(model.updateFileToTask.previousTask){
					            	trace(" TASKSCOMMAND updateFileToTask previousTask :"+model.updateFileToTask.previousTask.taskId);
					            	if(model.updateFileToTask.previousTask.fileObj){
					            		trace("TASKSCOMMAND updateFileToTask previousTask fileObj :"+model.updateFileToTask.previousTask.fileObj.fileId);
					            	}
					            }
					        }	 	 */
