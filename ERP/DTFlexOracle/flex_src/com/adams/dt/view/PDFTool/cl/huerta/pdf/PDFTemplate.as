...
package com.adams.dt.view.PDFTool.cl.huerta.pdf
{
	...
	public class PDFTemplate extends PDF
	{
		...
		public function PDFTemplate(orientation:String='Portrait', unit:String='Mm', pageSize:Object=null, rotation:int=0)
		{
	    ...
	    public function beginTemplate(x:Number=0, y:Number=0, w:Number=-1, h:Number=-1):Number {
			...
			template.o_bMargin = this.bMargin;
			template.o_tMargin = this.tMargin;
			template.o_lMargin = this.lMargin;
			template.o_rMargin = this.rMargin;
	        ...
	        this.setRightMargin(this.currentPage.w-w+this.rMargin);
	    ...
	    public function endTemplate():* {
	        if (this._intpl) {
	            ...
	            this.tMargin = template.o_tMargin;
	            this.lMargin = template.o_lMargin;
	            this.rMargin = template.o_rMargin;
	     ...
	     */
	    override public function setFont(family:String, style:String='', size:int=0):void {
	        /**
	        ...
	        super.setFont(family, style, size);
	        ...
	        if (this._intpl) {
				...
				if (this._res['page'][this.currentPage.page] == undefined)
					this._res['page'][this.currentPage.page] = new Object();
				if (this._res['page'][this.currentPage.page].fonts == undefined || this._res['tpl'][this.currentPage.page].fonts == null)
					this._res['page'][this.currentPage.page].fonts = new Dictionary();

	            this._res['page'][this.currentPage.page].fonts[fontkey] = this.fonts[fontkey];
	        }
	    }
	     ...
	     * See AlivePDF Documentation ;-)
	     */
	    override public function addImageStream ( imageBytes:ByteArray, x:Number=0, y:Number=0, width:Number=0, height:Number=0, alpha:Number=1, resizeMode:String="None", blendMode:String="Normal", keepTransformation:Boolean=true, link:String='', isMask:Boolean=false ):void {
	    	super.addImageStream(imageBytes,x,y,width,height,alpha,resizeMode,blendMode,keepTransformation,link,isMask);
	        if (this._intpl) {
				if (this._res['tpl'] == undefined || this._res['tpl'] == null)
					this._res['tpl'] = new Array();
				if (this._res['tpl'][this.tpl] == undefined)
				...
				this._res['tpl'][this.tpl].extgs.push(this.extgstates.length-1);
	        } else {
				if (this._res['page'] == undefined || this._res['tpl'] == null)
					this._res['page'] = new Array();
				if (this._res['page'][this.currentPage.page] == undefined)
					this._res['page'][this.currentPage.page] = new Object();
				if (this._res['page'][this.currentPage.page].images == undefined || this._res['tpl'][this.currentPage.page].images == null)
					this._res['page'][this.currentPage.page].images = new Dictionary();

	            this._res['page'][this.currentPage.page].images[imageBytes] = this.streamDictionary[imageBytes];
	        }
	    }
	    ...
	    override public function addLink( x:Number, y:Number, width:Number, height:Number, link:*, highlight:String="I" ):void {
	        if (this._intpl)
	            throw new Error('Using links in templates aren\'t possible!');
	        super.addLink(x,y,width,height,link,highlight);
	    }
	    
	    override public function addBookmark( text:String, level:int, y:Number, color:RGBColor ):void {
	        if (this._intpl)
	    ...
	    protected function writeFormXObjects():void {
	        var filter:String=(this.compress) ? '/Filter /FlateDecode ' : '';
					...
					if (this._res['tpl'][tplidx] != undefined) {
						...
						if (template.extgs != undefined && template.extgs != null) {
			            	...
			            	for each( var gs:String in template.extgs ) {
								this.write('/GS'+gs+' '+extgstates[gs].n +' 0 R');
			            	}
			            	this.write('>>');
			            }
			  		}
        ...
        override protected function write( content:* ):void {
            if ( currentPage == null ) throw new Error ("No pages available, please call the addPage method first !");
