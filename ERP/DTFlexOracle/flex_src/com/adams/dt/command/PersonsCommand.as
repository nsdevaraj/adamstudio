package com.adams.dt.command
{
	...
	public final class PersonsCommand extends AbstractCommand 
	{
		...
		override public function execute( event:CairngormEvent ):void {	
			...
			switch( event.type ) {
				...
				case PersonsEvent.EVENT_LOGIN_RESULT:
					...
					trace("personsEvent. personId"+personsEvent.personId+" , "+model.person.defaultProfile);
		...
		private function getAllPersonsResultServer( rpcEvent : Object ) : void
		{
			...
			model.totalPersonColl = rpcEvent.result as ArrayCollection;
			var person:Persons
			model.personLogins = [];
			...
			trace("test....1"+model.personsTotalCollection.length+" , model.person.personLogin :"+model.person.personLogin);
			...
			try
			{
				...
				model.encryptorUserName = StringUtils.replace(escape(model.encryptor.encrypt(model.person.personLogin)),'+','%2B');
				model.encryptorPassword = StringUtils.replace(escape(model.encryptor.encrypt(model.person.personPassword)),'+','%2B');
				trace("Person :"+model.person.personFirstname);
				personLoginEvt.personId = model.person.personId;
				trace("PersonId:"+personLoginEvt.personId);
				model.ChatPerson = model.person;
				model.businessCard = model.person;
				model.dataReach = true; 
			}
		...
		private function getAllCollectionAfterLogin( rpcEvent : Object ) : void {
			...
			trace("*******************************************************************************************");
			trace("resultArrColl:"+resultArrColl.length);
			/*
			...
			*/
			 ...
			 */
			 ...
			 /*
			Events Sequences
				[compEvent,getAllPhaseTemp,getAllStatus,prof_event,
				getAllworkflowTemplate,getDomainEvent,profileevent,groupPer, domainworkflows, modules]
			*/ 
