	...
	public final class CategoriesCommand extends AbstractCommand 
	{
		...
		{	
			...
			switch(event.type){
				...
				case CategoriesEvent.EVENT_ORACLE_NEWPROJECTCALL:
					...
					//this.delegate.createProjectList("SUGUMAR",2,model.person.personId);
					
					//this.delegate.responder = new Callbacks(createOracleCategoryResult,fault);
					this.delegate.SelectALLCategories(2);					
					break; 
				//-----------------------------------
				//-----------------------------------
				//New Oracle Conversion by kumar     
				case CategoriesEvent.EVENT_ORACLE_NEWPROJECT:
					this.delegate = DelegateLocator.getInstance().pagingDelegate;
					categories2 =  model.categories2;
					categories1 = model.categories1;	
					delegate.responder = new Callbacks(createOracleResult,fault);	
					delegate.createFolderList(model.domain,categories1,categories2);	
					break; 
				//-----------------------------------
		...
		//New Oracle Conversion by kumar
		private function createOracleResult( rpcEvent : Object ) : void {
			super.result(rpcEvent);
			...
			trace("*******************************************************************************************");
			trace("createOracleResult resultArrColl:"+resultArrColl.length);			
			trace("*******************************************************************************************");
			
			 var categoryAllList:ArrayCollection 		= resultArrColl.getItemAt(0) as ArrayCollection;
			 trace("createOracleResult categoryAllList:"+categoryAllList.length);
		...
		private function createOracleProjectResult( rpcEvent : Object ) : void {
			...
			trace("*******************************************************************************************");
			 ...
			 var categoryAllList:ArrayCollection 		= resultArrColl.getItemAt(0) as ArrayCollection;
			 trace("createOracleProjectResult categoryAllList:"+categoryAllList.length);
			} 
		
		}
		private function createOracleCategoryResult( rpcEvent : Object ) : void {
			super.result(rpcEvent);
			var containerUtils: LoginUtils  = new LoginUtils();
			var categories : Categories = rpcEvent.result as Categories;
			if(categories){
			trace("*******************************************************************************************");
			trace("createOracleCategoryResult Categories:"+categories.categoryName);			
			trace("*******************************************************************************************");
