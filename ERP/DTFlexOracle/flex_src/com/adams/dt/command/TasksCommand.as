package com.adams.dt.command
{
	...
	{ 
		...
		{	 
		    ...
		    switch(event.type){    
		       ...
		       case TasksEvent.CREATE_AUTO_INITIAL_TASKS:
				...
				case TasksEvent.EVENT_ORACLE_NEWTASKCALL:
					this.delegate = DelegateLocator.getInstance().pagingDelegate;					
					this.delegate.responder = new Callbacks(createOracleTaskResult,fault);	
					this.delegate.createBulkTasks(1,3,25,2,"P3T01A");					
					break; 
				//-----------------------------------
		        default:
		        break; 
		...
		private function createOracleTaskResult( rpcEvent : Object ) : void {
			...
			if(resultArrColl){
			trace("*******************************************************************************************");
			trace("createOracleTaskResult resultArrColl:"+resultArrColl.length);			
			trace("*******************************************************************************************");
			
			 var categoryAllList:ArrayCollection 		= resultArrColl.getItemAt(0) as ArrayCollection;
			 trace("createOracleTaskResult categoryAllList:"+categoryAllList.length);
			} 
		...
		private  function commonProfileTaskResult( rpcEvent:Object ):void {	
			...
			for each(var item:Object in model.currentProjectFiles){
				...
				if(item.taskId!=null){
	   			...
	   			item.workflowId = model.currentTasks.workflowtemplateFK.workflowTemplateId;
		...
		public function createInitialTasksResult(rpcEvent : Object ) : void {	 
			model.createdTask = Tasks(rpcEvent.message.body);
			var pdfConver:Boolean = false;
			for each(var item:Object in model.currentProjectFiles){
				item.destinationpath = model.currentDir;
				item.projectId = model.project.projectId;
				item.taskId  =  model.createdTask.taskId;
				var filename:String = item.name; 
				var splitObject:Object = FileNameSplitter.splitFileName(item.name)
				/* if(splitObject.extension == 'pdf'&& !pdfConver){
					pdfConver = true;
					item.miscelleneous = 'IND'+model.currentTime.getTime();
				} */
				if(item.taskId!=null){
					filename = splitObject.filename+item.taskId+"."+splitObject.extension;
	   			}
			...
			for each(var refItem:FileDetails in model.refFilesDetails){
				...
				refItem.projectFK = model.project.projectId;
			...
			tasks.projectObject = model.project;
		...
		public function fileListDetails():void {
			...
			for each(var item:Object in model.currentProjectFiles){				
				...
				if(model.onlyEmail == 'REEMAIL') {
					item.taskId  =  model.TaskIDAttachArrayColl[0];	
				}
				
				item.fileName = item.name;
				var filename:String = item.name; 
				var splitObject:Object = FileNameSplitter.splitFileName(item.name)
					filename = splitObject.filename+model.TaskIDAttachArrayColl[0]+"."+splitObject.extension;
	   			item.storedName = filename;   			
				item.projectId = model.currentProjects.projectId;
				item.sourcePath = onUpload(item);
				
				var objFile:Object = new Object();
				objFile.destinationpath = item.destinationpath;
				objFile.storedName = item.storedName;
				objFile.projectId = item.projectId;				
				objFile.sourcePath = item.sourcePath;						

				objFile.fileName = item.name;
				objFile.filePath = item.destinationpath+"/"+item.type+"/"+item.storedName;	
	          	objFile.type = item.type;	  
				
				model.FileAttachArrayColl.addItem(objFile);
				model.filesToUpload.addItem(item);	
			}
			backUpload();
		}
		...
		{	 
			...
			}else if(model.updateProperty||model.currentTasks.workflowtemplateFK.profileObject.profileCode == "FAB")
			{
				var arrc : ArrayCollection = model.currentTasks.projectObject.propertiespjSet;
				for each(var items : Propertiespj in arrc)
				{
		...
		private function subProfileFileUpload(temptask:Tasks):void
		{
			for each(var item:Object in model.currentProjectFiles){
				...
				if(item.taskId!=null){
	   			...
	   			item.workflowId = temptask.workflowtemplateFK.workflowTemplateId;
				item.storedName = filename
				item.projectId = model.currentProjects.projectId;
				item.sourcePath = onUpload(item);
				model.filesToUpload.addItem(item);
