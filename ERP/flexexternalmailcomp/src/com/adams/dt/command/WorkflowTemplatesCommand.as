package com.adams.dt.command
{
	...
	{ 
		...
		{ 	
			...
			//model.messageTemplatesCollection = ArrayCollection(rpcEvent.result);
		
			/* var tevent:TasksEvent = new TasksEvent(TasksEvent.CREATE_MSG_TASKS);
			tevent.tasks = etasks;
				//tevent.tasks.personDetails = model.messageSender;
			//Tasks(tevent.tasks).workflowtemplateFK = Workflowstemplates(ArrayCollection(rpcEvent.result).getItemAt(0));
			tevent.tasks.workflowtemplateFK = Workflowstemplates(ArrayCollection(rpcEvent.result).getItemAt(0));
			trace("getMsgWorkflowTemplateesult :"+tevent.tasks.workflowtemplateFK.workflowTemplateId);
 			var handler:IResponder = new Callbacks(result,fault)
 			var tSeq:SequenceGenerator = new SequenceGenerator([tevent],handler)
  			tSeq.dispatch(); */
