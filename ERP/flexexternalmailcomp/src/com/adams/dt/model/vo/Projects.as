package com.adams.dt.model.vo
{
	...
	public final class Projects implements IValueObject
	{
		...
		public function set presetTemplateID (value:int):void
		{
			...
			trace("---inside project--"+_presetTemplateFK.propertiesPresetSet.length)
		}
		...
		private var _propertiespjSet : ArrayCollection = new ArrayCollection();
			 //depature fields vo
       	public var depatureFields:DepatureFields = new DepatureFields();
		public function set propertiespjSet (value : ArrayCollection) : void
		{
			_propertiespjSet = value;
			for each (var item:Propertiespj in value){
				if(item.propertyPreset.fieldName=="print_process"){
					procedeImpression = Number(item.fieldValue);
				}
				if(depatureFields.hasOwnProperty(item.propertyPreset.fieldName)){
					depatureFields[item.propertyPreset.fieldName] = item.fieldValue;
				}
			}
