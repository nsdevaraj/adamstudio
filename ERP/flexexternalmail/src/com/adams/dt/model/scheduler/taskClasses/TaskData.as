package com.adams.dt.model.scheduler.taskClasses
{
	...
	{	
				...
				if( str == "Archive" ) {
					if( Projects( dataCollection.getItemAt( i ) ).projectDateEnd != null ) {
						...
						if( ( archiveStartDate != null ) && ( archiveEndDate != null ) ) {
							var archiveEntry : SimpleTask = createColoredEntry( i, archiveStartDate, archiveEndDate, 0, "", false, 0, projectNames[ i ],"","","",null,"",-1, false );
					...
					for( var j:int = 0; j < phaseArray_Len; j++ ) {
						...
						if( ( startDate != null ) ) {
							if( endDate == null ) {
								endDate = new Date();
								entryColor = 0xFFFF00;
								var entry : SimpleTask = createColoredEntry( j, startDate, endDate, entryColor, entryLabel, false, phaseBelonging, projectNames[ i ],"","","",null,"",-1, false );
								row.addItem( entry );
								break;
							}
							else {
								var myEntry : SimpleTask = createColoredEntry( j, startDate, endDate, entryColor, entryLabel, false, phaseBelonging, projectNames[ i ],"","","",null,"",-1, false );
								row.addItem( myEntry );
							}
						}
					}
			...
			for( var j:int = 0; j < rows_Len; j++ ) {
				...
				if( ( startDate != null ) && ( endDate != null ) ) {
					...
					var entry : SimpleTask = createColoredEntry( j, startDate, endDate, entryColor, entryLabel, false, 0, "", entryComment, entryPath, taskLabel, taskBcard, 
					                                             Tasks( dataCollection.getItemAt( j ) ).workflowtemplateFK.profileObject.profileLabel, ( taskList.length - 1 ), currentTask );
					row.addItem( entry ); 
				}
			}
			result.push( row );
		...
		private function createColoredEntry( i : Number, startDate : Date, endDate : Date, color : uint, label:String, finalTask:Boolean, val:int, name:String,
																   comment:String, path:String, taskLabel:String, taskBCard:TaskBusinessCard, profileTask:String, id:int, currentTask:Boolean ) : SimpleTask
		{
