...
{
	import com.adams.dt.event.scheduler.LayoutScrollEvent;
	import com.adams.dt.model.ModelLocator;
	import com.adams.dt.model.scheduler.taskClasses.ITask;
	import com.adams.dt.model.scheduler.taskClasses.TaskLayoutItem;
	import com.adams.dt.model.scheduler.taskClasses.task_internal;
	...
	public final class TaskNavigator extends EventDispatcher
	{
		...
		public function TaskNavigator( owner : PhaseViewer )
		{
			this.owner = owner;
		...
		public function set pixelScrollEnabled( value : Boolean ) : void
		{
			...
			dispatchEvent( new Event( TaskNavigator.INVALIDATE_DISPLAY_LIST ) );
		}
		...
		public function moveToTime( milliseconds : Number , totalMilliseconds : Number ) : void
		{
			var position : Number = milliseconds * entryLayoutImpl.contentWidth / totalMilliseconds;
			animateHorizontalScrollPosition( position );
		}

		public function gotoEntry( entry : ITask ) : void
		{
			var milliseconds : Number = entry.startDate.getTime() - entryLayoutImpl.startDate.getTime();
			gotoTime( milliseconds );
			var item : TaskLayoutItem = entryLayoutImpl.findLayoutItem( entry );
			owner.yPosition = item.y;
		}

		public function moveToEntry( entry : ITask ) : void
		{
			var milliseconds : Number = entry.startDate.getTime() - entryLayoutImpl.startDate.getTime();
			moveToTime( milliseconds , entryLayoutImpl.totalMilliseconds );
			var item : TaskLayoutItem = entryLayoutImpl.findLayoutItem( entry );
		...
		private function animateVerticalScrollPosition( position : Number ) : void
		{
			...
			e.duration = owner.getStyle( "moveDuration" );
			e.easingFunction = owner.getStyle( "moveEasingFunction" ) as Function;
			e.addEventListener( EffectEvent.EFFECT_END , verticalAnimationEnd , false , 0 , true);
			e.play();
		}
