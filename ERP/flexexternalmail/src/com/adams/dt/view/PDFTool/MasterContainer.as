package com.adams.dt.view.PDFTool
{
	...
	public class MasterContainer extends Canvas
	{
		...
		override protected function createChildren():void
		{
			...
			singlePage.addEventListener(SinglePageCanvas.PAGE_ASSETS_PROPERTIES_CHANGE, onPagePropertiesChange);
			singlePage.addEventListener(SinglePageCanvas.ZOOM_IN, onZoomIn);
			singlePage.addEventListener(SinglePageCanvas.ZOOM_OUT, onZoomOut);
			...
			BindingUtils.bindProperty(singlePage, "img2ByteArray", model, "modelINDByteArrray");
			...
			this.addEventListener(FlexEvent.CREATION_COMPLETE, function ():void{linkFunc()});
			
			setStyle("backgroundColor","#424242");
			setStyle("backgroundAlpha","1");
			
			this.addEventListener(EffectEvent.EFFECT_END, onEffectEnd);
			this.addEventListener(ResizeEvent.RESIZE, onResizeAction);
		...
		private function onDataProviderChange(event:CollectionEvent):void
		{
			...
			for(i=0;i<event.target.length; i++)
			{
				...
				if(commentDetail.history == 0){
					...
					if(cType != SinglePageCanvas.SHAPE_NOTE){
						...
						commentBox.addEventListener(CommentItem.COMMENT_MOVE, function (event:Event):void{ linkFunc(); });
						...
						if(cPDF!=SinglePageCanvas.PDF1){
							commentItem.addEventListener(CommentItem.COMMENT_MOVE, function (event:Event):void{ linkFunc(); });
						}
						//
						...
						trace(" commentDetail.commentX "+commentDetail.commentID);
						trace(" commentDetail.commentY "+commentDetail.commentY);
			...
			trace("externalCanvas.numChildren : "+externalCanvas.numChildren)
		...
		private function linkFunc():void
		{
