package com.adams.dt.view.PDFTool
{
	...
	public class SinglePageCanvas extends Canvas
	{
		...
		private var _img2ByteArray:ByteArray;
		[Bindable]
		public function set img2ByteArray (value:ByteArray):void
		{
			_img2ByteArray = value;
			img2.imgSourceBitmapData=value;
		...
		public function get img2ByteArray ():ByteArray
		{
			return _img2ByteArray;
		}
		...
		override protected function createChildren():void
		{
			...
			this.addEventListener(MouseEvent.MOUSE_OUT,onMouseOutOver);
			this.addEventListener(MouseEvent.MOUSE_OVER,onMouseOutOver);
			
			this.addEventListener(MouseEvent.MOUSE_WHEEL,onMouseWheel);
			
			// Register Event Listener for KEYBOARD action 
			this.addEventListener(KeyboardEvent.KEY_DOWN,onKeyEvent); 
			this.addEventListener(KeyboardEvent.KEY_UP,onKeyEvent); 
			
			pdfToggleTimer.addEventListener(TimerEvent.TIMER, onPdfToggleTimer, false , 0 , true);
			
			regionPointer.source = null;//ImageResourceEmbedClass.REGION_COMPARE;
			regionPointer.width = 95;
			regionPointer.height = 120;
			
			regionPointer.visible = false;
			
			img1.addEventListener(ImageSWF.IMAGE_LOAD_COMPLETE,onImageLoadComplete);
			img2.addEventListener(ImageSWF.IMAGE_LOAD_COMPLETE,onImageLoadComplete);
			
			regionPointer.addEventListener(MouseEvent.MOUSE_DOWN, onRegionPointerMouseAction);
        ...
        public function resetAll():void
		{
			...
			var sVal:Number = imgHeight / this.parent.height;
			if(sVal!=0){
			//scaleAt(0.9+(1-rotateScaleValue()), 0, 0); 
				if(this.parent.height>imgHeight*rotateScaleValue()+100)
				{
					fitToHeight(true);
				}
				else//(this.parent.height<imgHeight*rotateScaleValue())
				{
					fitToHeight(false);
				}
			}
		...
		private function onMouseUp( event : MouseEvent ) : void
		{
			...
			if(panSelect)
			{
				...
				//CursorManager.setCursor(ImageResourceEmbedClass.CURSOR_OPEN_ICON, CursorManagerPriority.HIGH);
			}
			else{
				if(this.focusAction==NOTE && !this.dragStart && drawingStart)
				{
					if(Math.abs(startX-stopX) > minimumNoteWidth || Math.abs(startY-stopY) > minimumNoteHeight)
					{
						...
						commentEvent = new CommentEvent(CommentEvent.ADD_COMMENT, null, "", "", cXpos , cYpos ,cWidth,cHeight, "0x"+convertToRGB(this.fillColor), "2", cType, false,ModelLocator.getInstance().person.personId, Math.random()*200, Math.random()*200, true,GetVOUtil.getProfileObject(model.person.defaultProfile).profileCode);
		...
		public function rotateAt( angle : Number, originX : Number, originY : Number ) : void
        {
            ...
            dispatchEvent(new Event(PAGE_ASSETS_PROPERTIES_CHANGE));
            trace("Rotate Value : "+rotateScaleValue());
        }
        ...
        private function onKeyEvent(event:KeyboardEvent):void
		{
			...
			if(event.type==KeyboardEvent.KEY_DOWN)
			{
				if(event.keyCode == 32){
					//if(!dragStart)
					if(!panSelect)
					//CursorManager.setCursor(ImageResourceEmbedClass.CURSOR_OPEN_ICON);
		...
		private function drawShape(shape:String, mouseType:String):void
		{
			trace("Start X : "+drawStartX+" ::: "+drawStartY+" :: "+drawStopX+" :: "+drawStopY)
				...
				if(mouseType == MouseEvent.MOUSE_UP)
				{
					...
					{
						var commentEvent:CommentEvent;
						commentEvent = new CommentEvent(CommentEvent.ADD_COMMENT, null, svgData, shape.substr(0,1).toUpperCase()+shape.substr(1) ,"0","0","0","0","0","2", "4", false, ModelLocator.getInstance().person.personId, 0, 0, false, GetVOUtil.getProfileObject(model.person.defaultProfile).profileCode);
						commentEvent.dispatch(); 
		...
		private function onRegionPointerMouseAction(event:MouseEvent):void
		{
			if(event.type == MouseEvent.MOUSE_DOWN){
				...
				regionPointer.addEventListener(MouseEvent.MOUSE_MOVE, onRegionPointerMouseAction);
				regionPointer.addEventListener(MouseEvent.MOUSE_UP, onRegionPointerMouseAction);
			}
		...
		public function DrawNoteCircle(x:Number,y:Number,NoteNumber:Number):void{
			...
			canvas.graphics.beginFill(0xFFFF00, 0.5);
