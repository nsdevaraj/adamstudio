package com.adams.dt.view.PDFTool
{
	...
	public class CommentBox extends Canvas
	{
		...
		override protected function createChildren():void
		{
			...
			miniMaxBtn.addEventListener(MouseEvent.CLICK, onMouseClick_MinMax);
			addEventListener(EffectEvent.EFFECT_END, onEffectEnd);
			moveCanvas.addEventListener(MouseEvent.MOUSE_DOWN, onMouseAction);
			
			historyCollection.addEventListener(CollectionEvent.COLLECTION_CHANGE,historyCollectionChange);
			historyStepper.addEventListener(NumericStepperEvent.CHANGE,stepperChange);
			
			setChildIndex(moveCanvas, this.numChildren-5);
			
			editBtn.addEventListener(MouseEvent.CLICK, editSaveMouseClick);
			editBtn.toggle = true;
			
			deleteBtn.addEventListener(MouseEvent.CLICK, deleteMouseClick);
        ...
        private function onMouseAction(event:MouseEvent):void
        {
        	if(event.type == MouseEvent.MOUSE_DOWN)
        	{
        		...
        		moveCanvas.addEventListener(MouseEvent.MOUSE_UP, onMouseAction);
        		moveCanvas.addEventListener(MouseEvent.MOUSE_MOVE, onMouseAction);
        	}
