	...
	public class Utils
	{
		...
		public static function getPropertyPj( itemId:int,ppj:ArrayCollection):Propertiespj {
			for each(var propPj:Propertiespj in ppj){
				//trace("UTILS :"+propPj.propertyPreset.propertyPresetId+" == "+itemId);
				trace("UTILS :"+propPj.propertyPjId+" , "+propPj.projectFk);
				if(propPj.propertyPreset.propertyPresetId == itemId){
					trace("UTILS return :"+propPj);
					return propPj;
				}
			}
			return null;
				...
				for each( var item:Object in taskCollection ) {
					if( item.domain.categoryId != null ) {
						if( item.domain.categoryId == domain.categoryId ){
							...
							var projects:Projects = Utils.getSpecificProject( tasks.projectObject.projectId ); 
		...
		public static function makeData( projectObj:Projects, obj:Object, propertySet:Array, headerSet:Array ):void {
			...
			for( var j:int = 0; j < propertySet.length; j++ ) {
				if( propertySet[ j ] == 'name' ) {
					obj[ headerSet[ j ] ] = projectObj.projectName.substring( ( projectObj.projectName.indexOf( '_' ) + 1 ), projectObj.projectName.length );
				}
				else if( propertySet[ j ] == 'refId' ) {
					obj[ headerSet[ j ] ] = projectObj.projectName.split( '_' )[ 0 ];
				} 
					...
					obj[ headerSet[ j ] ] = GetVOUtil.getWorkflowTemplate( projectObj.wftFK ).profileObject.profileLabel;
				}
				else {
		...
		/* public static function getImpremiur( id:int ):String {
 		...
 		public static function matchingProjects( dataCollection:ArrayCollection ):ArrayCollection {
			...
			for( var i:int = 0; i < dataCollection.length; i++ ) {
				var prjName:String = dataCollection.getItemAt( i ).ReferenceId + '_' + dataCollection.getItemAt( i ).ReferenceName; 
		...
		public static function getSpecificProject( prjId:int ):Projects {
			...
			var item:Projects = new Projects();
		...
		public static function modifyItem( item:Propertiespj, arrc:ArrayCollection ):int {
			var returnValue:int = -1;
			var sort:Sort = new Sort(); 
		    sort.fields = [ new SortField( 'propertyPjId' ) ];
		    arrc.sort = sort;
		    arrc.refresh(); 
		...
		public static function modifyItems( oldList:IList, newList:IList ):ArrayCollection {
			for ( var i:int = 0; i < newList.length; i++ ) {
				var isItem:int =  modifyItem( Propertiespj( newList.getItemAt( i ) ), oldList as ArrayCollection );
				if( isItem != -1 ) {
					oldList.removeItemAt( isItem );
					oldList.addItemAt( newList.getItemAt( i ), isItem );
				}
				else {
					oldList.addItemAt( newList.getItemAt( i ), i );
				}	
			}
