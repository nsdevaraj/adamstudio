package com.adams.dt.command
{
	...
	import com.adams.dt.event.PDFTool.ImgEvent;
	...
	{ 
		...
		{	 
			...
			switch(event.type){    
				...
				case ImgEvent.LOADING_COMPLETED: 
					var imgEvent : ImgEvent = event as ImgEvent;
					if(imgEvent.img == ImgEvent.IMG1)
					{
						model.pdfDetailVO.pdfFileBitmapData1 = imgEvent.ImgBitmapData;
						model.pdfDetailVO.pdfWidth = imgEvent.ImgWidth;
						model.pdfDetailVO.pdfHeight = imgEvent.ImgHeight;
						model.pdfDetailVO.detailLoadedStatus = PDFDetailVO.IMAGE_LOADING_STATUS_COMPLETED;
					}
		
					if(imgEvent.img == ImgEvent.IMG2)
					{
						model.pdfDetailVO.pdfFileBitmapData2 = imgEvent.ImgBitmapData;
					}
					break; 
				...
				case PDFInitEvent.PDF_INIT:
					...
					{
						if(model.currentProfiles.profileCode == 'FAB' || model.currentProfiles.profileCode == 'CLT')
						{
							...
							// model.compareTasksCollection
							//Alert.show("----"+model.compareTasksCollection.length);
							...
							if(model.compareTask.taskId!=0&&model.pdfFileCollection.length>0){								
								model.compareTask.taskFilesPath = 'assets/iBrief/skins/report/preloader.png';
						...
						if(model.pdfFileCollection.length>0){
							...
							model.currentTasks.previousTask.taskFilesPath = 'assets/iBrief/skins/report/preloader.png';
						}
						...
						CairngormEventDispatcher.getInstance().dispatchEvent( commentEvent );
		...
		private function getCompareTask(_task : Tasks) : Tasks
		{
			...
			if(_task.previousTask != null)
			{
				...
				||_task.workflowtemplateFK.taskLabel=="COMPLEMENTS DE RELECTURE"||_task.workflowtemplateFK.taskLabel=="RELECTURE V1")
				{
					task = new Tasks()
		...
		public function addCommentResult( rpcEvent : Object ) : void
		{
			...
			trace("\n\n\n addCommentResult : : : : : : "+arrc.length);
			model.pdfDetailVO.commentListArrayCollection = arrc;
			model.pdfDetailVO.commentListArrayCollection.refresh();
			trace("\n\n\n model.pdfDetailVO.commentListArrayCollection : : : : : : "+model.pdfDetailVO.commentListArrayCollection.length);
			super.result(rpcEvent);
		}
		public function createCommentResult( rpcEvent : Object ) : void
		{
			super.result(rpcEvent);
			trace(rpcEvent.message.body+ 'rpcEvent.message.body')
			var commentVo : CommentVO = CommentVO(rpcEvent.message.body);
