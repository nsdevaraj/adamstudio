...
{ 
	...
	import com.adams.dt.view.PDFToolSkinView;
	import com.adams.dt.view.components.PDFTool.*;
	...
	{ 		 
		...
		{
			_homeState = value;
			if(value==Utils.PDFTOOL_INDEX) addEventListener(Event.ADDED_TO_STAGE,addedtoStage,false,0,true);
		}
		...
		public function currentFileData(value:ByteArray, fileId:int, fileReleaseVersion:int, pageAC:ArrayCollection = null):void
		{
			if(fileId!=-1){
		...
		protected function commentCollectionChangeHandler(event:CommentCollectionEvent):void
		{
			...
			trace("event.commentItemName : "+event.commentItemName)
		...
		private function pageListChangeHandler(event:IndexChangeEvent):void
		{
			...
			//trace((view.pageListDG.selectedItem as FileDetails).fileId)
