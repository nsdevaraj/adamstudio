package com.adams.pdf.view.components.PDFTool
{
	
	import com.adams.pdf.view.components.PDFTool.CommentItem;
	...
	[Event (type="commentCollectionUpdate",type="com.adams.pdf.view.components.PDFTool.events.CommentCollectionEvent")]
	public class ShapeComponentFXG extends Group
	{
		...
		private function mouseAction(event:MouseEvent):void
		{
			...
			if(event.type == "mouseUp") {
				...
				if(shapeSelected != NONE)
				{
					...
					commentVO.commentTitle = shapeSelected;
					commentVO.commentDescription = Utils.convertToByteArray(fxgData);
		...
		{
			if(event.currentTarget is Group && _deleteItem){
				this.removeElement(Group(event.currentTarget));
			}
		}
