...
/**
 * FLARLabeling_ARToolKitと同じ処理をするけど、エリア計算にintを使う。 
 * 画面サイズが1600ｘ1600を超えると挙動が怪しくなる。
 * 
 */
public class FLARLabeling_ARToolKit_X2 implements IFLARLabeling
{
	private static final int WORK_SIZE = 1024 * 32;// #define WORK_SIZE 1024*32

	private final FLARWorkHolder work_holder = new FLARWorkHolder(WORK_SIZE);

	private FLARIntSize _dest_size;

	private IFLARLabelingImage _out_image;

	public void attachDestination(IFLARLabelingImage i_destination_image) throws FLARException
	{
		// サイズチェック
		FLARIntSize size = i_destination_image.getSize();
	...
	public IFLARLabelingImage getAttachedDestination()
	{
	 ...
	 */
	public void labeling(FLARBinRaster i_raster) throws FLARException
	{
		int m, n; /* work */
		int i, j, k;
		IFLARLabelingImage out_image = this._out_image;

		// サイズチェック
		FLARIntSize in_size = i_raster.getSize();
	...
	private void updateLabelStackLarge(FLARLabelingLabelStack i_stack, int[] i_lindex, FLARIntSize i_size, int[][] i_work, int i_work_max, int i_number_of_label) throws FLARException
	{
		//計算用のワークを確保
		...
		// エリアと重心、クリップ領域を計算
		final FLARLabelingLabel[] labels = (FLARLabelingLabel[])i_stack.getArray();
		...
		for (int i = 0; i < i_number_of_label; i++) {// for(int i = 0; i < *label_num; i++ ) {
			final FLARLabelingLabel label_pt = labels[i];
			final int temp_ptr[] = temp[i];
