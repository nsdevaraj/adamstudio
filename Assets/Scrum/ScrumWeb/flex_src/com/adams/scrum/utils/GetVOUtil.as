...
{  
	...
	{  
		...
		/**
		 * @obj The Sprint for which the team member list has to be arrayed
		 * @personId The Current personID which is not going to be included
		 * Returns the teammembers( personIDs ) of the corresponding sprint by excluding
		 * the current personID  
		 * */
		public static function getSprintMembers( obj:Sprints, personId:int ):Array {
			var receivers:Array = [];
			for each( var member:Teammembers in  obj.teamObject.teamMemberSet ) {
				if( member.personFk != personId ) {
					receivers.push( member.personFk );
				}
			}
			return receivers;
		}
