package com.adams.scrum.views.mediators
{
	...
	public class SprintOpenViewMediator extends AbstractViewMediator
	{
		...
		override protected function init():void
		{
			...
			view.storyList.addEventListener(DragEvent.DRAG_ENTER , dragEnterTasks);
			view.storyList.addEventListener(DragEvent.DRAG_OVER , dragOverTasks);
		}
		//******************************************************************************************************************
		...
		/**
		 * function wwill trigger while drag enter will start...
		 **/
		private function dragEnterTasks(event:DragEvent):void
		{
			var dragObj:Array = event.dragSource.dataForFormat("treeDataGridItems") as Array; 
			DragManager.acceptDragDrop(view.storyList);
		}
		/**
		 * it is used to check the drop accept position of the
		 * advancec datagrid. if the stories has same task then 
		 * it won't allow the user to drop the image
		 **/
		private function dragOverTasks(event:DragEvent):void
		{
			var dragObj:Array = event.dragSource.dataForFormat("treeDataGridItems") as Array; 
			var rollOverIndex:int = view.storyList.calculateDropIndex(event);
			
			
			var rowsDisplayed:Number = view.storyList.rowCount;
			var topvisibleIndex:int  = view.storyList.verticalScrollPosition;
			var botvisibleIndex:int  = topvisibleIndex + rowsDisplayed;
			if( rollOverIndex <= topvisibleIndex) {
			view.storyList.verticalScrollPosition = Math.max( view.storyList.verticalScrollPosition- 1, 0 );
			} else if( rollOverIndex >= botvisibleIndex - 1 ){
			view.storyList.verticalScrollPosition += 1;
			}
			
			var rollOverPositionObject:Object = view.storyList.indexToItemRenderer(rollOverIndex).data
			if(rollOverPositionObject!=null)
			{
				if(rollOverPositionObject is Stories)
				{
					droppedParentStories = rollOverPositionObject as Stories
				}else{
					droppedParentStories = view.storyList.getParentItem(rollOverPositionObject);
				}
				if( !checkItemsFound( droppedParentStories, Tasks(dragObj[0] )) ){
		...
		[Bindable]
		private var droppedParentStories:Stories;
		[Bindable]
		private var dropedIndex:int;
		private var sortabale:Boolean
		/**
		 * Dropped the task in the particular . it will created the new task ann
		 * fries the signal to update the advanced datagrid...
		 **/
		private function dragAndDropTasks(event:DragEvent):void
		{
			var droppedTaskSignal:SignalVO = new SignalVO(this,taskDAO,Action.CREATE);
			childCol = IHierarchicalCollectionView(view.storyList.dataProvider).getChildren(droppedParentStories);
			var bulkTasksCollection:ArrayCollection = new ArrayCollection();
			var dragObj:Array = event.dragSource.dataForFormat("treeDataGridItems") as Array;
			var draggedTasks:Tasks = Tasks(dragObj[0])
			var addedTask:Tasks = new Tasks();
			dropedIndex = view.storyList.calculateDropIndex(event);
			...
			(childCol.length>0)?sortabale=false:sortabale=true;
			addedTask.taskId = NaN;
			addedTask.storyFk = droppedParentStories.storyId;
			...
			selectedStory = droppedParentStories;
			
			
			droppedTaskSignal.valueObject = addedTask; 
			signalSeq.addSignal(droppedTaskSignal)
		}
		[Bindable]
		public var childCol:ICollectionView;
		/**
		 * 
		 **/
		private function adSorting():void
		{
			var sort:Sort = new Sort();
			sort.fields = [new SortField("storyId")];
			childCol.sort = sort;
			childCol.refresh();
			IHierarchicalCollectionView(view.storyList.dataProvider).refresh();
		}
		/**
		...
		override protected function serviceResultHandler(obj:Object,signal:SignalVO):void {
			...
			if( signal.destination == taskDAO.destination ){
				if( signal.action == Action.CREATE ){
					...
					// Update the task to product story Collection..
					var createProductStories:Stories = GetVOUtil.getValueObject(selectedStory,storyDAO.destination,currentInstance.currentProducts.storyCollection) as Stories;
					Utils.addArrcStrictItem(obj,createProductStories.taskSet,taskDAO.destination);
					Utils.addArrcStrictItem(createProductStories,currentInstance.currentProducts.storyCollection,storyDAO.destination,true);
				...
				if( signal.action == Action.UPDATE ){ 
					...
					// Update the task to product story Collection..
					var updateProductStories:Stories = GetVOUtil.getValueObject(selectedStory,storyDAO.destination,currentInstance.currentProducts.storyCollection) as Stories;
					Utils.addArrcStrictItem(selectedTask,updateProductStories.taskSet,taskDAO.destination, true);
				...
				if( signal.action == Action.DELETE ){  
					...
					// Remove the task from the Product Story Collection ....
					var deleteProductStories:Stories = GetVOUtil.getValueObject(selectedStory,storyDAO.destination,currentInstance.currentProducts.storyCollection) as Stories;
					Utils.removeArrcItem(selectedTask,deleteProductStories.taskSet,Utils.TASKKEY);
